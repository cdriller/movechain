...
contract MoveChain is EIP712 {
    ...
    uint256 ethToCredit;
    uint256 creditToEth;    // if we want to do a different back-conversion-rate (e.g. take a part for ourselves), we could also just use it by 1/ethToCredit

    struct Operator {
        bool whitelisted;
        uint256 price;
        uint256 totalEarningsInEth; // just stored on-chain because of the assignment, I don't really think, this is necessary
        string name;
    }
    ...
    // is this redundant with creditsOf()?
