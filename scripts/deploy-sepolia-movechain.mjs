...
const params = JSON.parse(readFileSync(join(root, PARAMS), "utf8"));
const moveChain = addresses["MoveChainOnlyModule#MoveChain"];
const tripCredits = params.MoveChainOnlyModule?.tripCredits;
if (!moveChain) {
  console.error(`Could not find MoveChainOnlyModule#MoveChain in ${DEPLOYMENTS}`);
  process.exit(1);
}
if (!tripCredits) {
  console.error(`Could not find MoveChainOnlyModule.tripCredits in ${PARAMS}`);
  process.exit(1);
}
